...
def non_max_suppression(prediction, 
    ...
    for i in range(len(prediction)):
        ...
        cls_idx = np.argmax(pred_item[5:])          # 获得类别置信度最大值的索引
        cls_conf = pred_item[5 + cls_idx]
        conf = obj_conf * cls_conf                  # 置信度=目标置信度*类别置信度（跟训练保持一致）
        if conf > conf_thres:                       # 筛选阈值以上的检测框
            cbox = pred_item[:4] 
            box = box_cxcywh_to_xyxy(cbox, offset)  # xywh -> xyxy
    ...
    order = np.argsort(det_conf)[::-1]              # 根据置信度降序排序的索引

    keep = []
    while len(order) > 0:

        if len(keep) > max_obj_num:                 # 超过目标数量最大值
            break

        top1_idx = order[0]
        top1_box = det_boxes[top1_idx]
        keep.append(top1_idx)                       # 将置信度最高的元素索引添加到保留数组中
        order = np.delete(order, 0)            # 然后才从原数组中删除

        inds = []
        for k in range(len(order)):
            topk_idx = order[k]
            topk_box = det_boxes[topk_idx]
            iou = box_iou(top1_box, topk_box)       # 剩下的检测框依次与置信度最高的框计算IOU
            if iou <= iou_thres:
                inds.append(k)                      # 保留IOU低于阈值的检测框
        order = order[inds]
